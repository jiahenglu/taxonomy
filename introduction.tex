...
``\textsf{Bill}'' is a short form of ``\textsf{William}'', and ``\textsf{ICDE}'' is an abbreviation of  ``\textsf{International Conference on Data Engineering}''.  Given two collections of strings,  the synonym-based similarity function can utilize the available synonyms to find string pairs which are semantically similar. While those methods improve the effectiveness of string joins, a synonym dictionary does not contain information such as
...
consider two tables in Figure \ref{fig:twotables}, which contains the information about the products of cellphones.  Consider the following SQL script:
